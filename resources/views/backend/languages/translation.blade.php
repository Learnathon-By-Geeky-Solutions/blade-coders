<x-backend-layout :title="__('Translation')">
            ...
            <form method="POST" action="{{ route('languages.translation', $language->id) }}">
                ...
                <div id="translations-container">
                    @foreach ($translations as $key => $translation)
                        <div class="translation-item relative grid grid-cols-2 gap-4">
                            <div class="mb-3">
                                <x-text-input name="translations[{{ $loop->index }}][key]" type="text"
                                    :value="$key" required autofocus autocomplete="off" placeholder="Enter Key"
                                    readonly />
                            </div>
                            <div class="mb-5">
                                <x-text-input name="translations[{{ $loop->index }}][value]" type="text"
                                    :value="$translation" required autofocus autocomplete="off" placeholder="Enter Value" />
                            </div>
                            <div class="absolute right-2.5 top-2.5">
                                <x-action-button class="remove-translation-btn" type="button" color="red"
                                    icon="trash" />
                            </div>
                        </div>
                    @endforeach
                ...
                <div class="flex justify-center gap-4">
                    <x-secondary-button id="add-translation" type="button">{{ __('Add Another Translation') }}</x-secondary-button>
            ...
            $(function() {
                ...
                $('#add-translation').click(function() {
                    ...
                    newTranslationItem.find('input[name^="translations"]').each(function() {
                        ...
                        name = name.replace(/\[\d+\]/,
                            `[${counter}]`); // Replace the old index with the new counter
                        $(this).attr('name', name);
                    });
                    ...
                    newTranslationItem.find('input[name$="[key]"]').removeAttr('readonly').attr('required',
                        true); // Make key field editable and required
                    newTranslationItem.find('input[name$="[value]"]').attr('required',
                        true); // Make value field required
