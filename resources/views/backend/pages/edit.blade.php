...
<x-backend-layout :title="__('Custom Page')">
    <div class="mb-4 flex items-center justify-between border-b border-gray-300 pb-4">
        <h1 class="inline-block text-xl font-semibold leading-6">{{ __('Update Custom Page') }}</h1>
    ...
    <x-card>
        <div class="p-4">
            <form id="page-form" method="POST" action="{{ route('pages.update', $page->slug) }}" enctype="multipart/form-data">
                @csrf
                @method('PATCH')
                <div>
                    <div class="mb-3">
                        <x-input-label for="title" :value="__('Title')" />
                        <x-text-input name="title" type="text" :value="old('title', $page->title)" required autocomplete="off" placeholder="{{ __('ex: Terms of Service') }}" />
                        <x-input-error class="error mt-2" :messages="$errors->get('title')" />
                    </div>
                    <div class="mb-3 mt-4">
                        <x-input-label for="subtitle" :value="__('Sub Title')" />
                        <x-textarea-input name="subtitle" type="text" :value="old('subtitle', $page->subtitle)" autocomplete="off"/>
                    ...
                    <div class="mb-0 mt-4 inline-flex w-full flex-col gap-3 md:flex md:flex-row md:items-center">
                        ...
                        <div class="flex-[3]">
                            <div class="flex items-center">
                                <div class="me-3">
                                    <img class="h-20 w-80 rounded-lg" src="{{ $page->banner->path }}" alt="img"/>
                                </div>
                                <div>
                    ...
                    <div class="mb-3 mt-4">
                        ...
                        <input id="quill-data" name="content" type="hidden" value="{{ old('content', $page->content) }}"/>
    ...
    @push('scripts')
        ...
        <script>
            ...
            const initialContent = @json(old('content', $page->content));
